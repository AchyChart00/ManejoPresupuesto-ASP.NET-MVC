...
namespace ManejoPresupuesto.Controllers
{
    public class TiposCuentasController : Controller
    {
        ...
        {
            var usuarioId = servicioUsuarios.ObtenerUsuarioId();
            var tipoCuenteExiste = await repositorioTiposCuentas.ObtenerPorId(tipoCuenta.Id, usuarioId);
            if (tipoCuenteExiste is null)
            {
                return RedirectToAction("NoEncontrado", "Home");
            }
        ...
        public async Task<IActionResult> Ordenar([FromBody] int[] ids)
        {
            ...
            var tiposCuentasOrdenados = ids.Select(
                (valor, indice)=> new TipoCuenta() { Id =valor, Orden= indice + 1}
                ).AsEnumerable();
